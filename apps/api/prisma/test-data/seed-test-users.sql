-- Test data: 20 utenti generati con Postgres `generate_series()`. Servono
...
--
-- `lpad(i, 3, '0')` produce "001", "002", ..., "020": così il sort
...
-- 2) Insert massivo: 20 righe da una sola query.
...
WHERE "email" IN (
    'user.010@example.com',
    'user.011@example.com'
);

COMMIT;

-- Verifica:
--   SELECT COUNT(*) FROM "User" WHERE id LIKE 'test-user-%';   -- atteso: 20
