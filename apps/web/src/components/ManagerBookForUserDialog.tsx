...
export interface BookForUserTarget {
  ...
  // (per cambiarlo il manager esce e cambia tab — più chiaro).
  type: SpotType;
  // Data preimpostata se il manager è arrivato da un click su una cella del
...
export function ManagerBookForUserDialog({ target, users, onClose, onSuccess }: Props) {
  ...
  async function handleSubmit() {
    ...
    setError(null);
    try {
      await api.managerCreateReservation({ userId, spotId, date });
      onSuccess();
      onClose();
    } catch (e) {
    ...
    >
      {target && (
        <div style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
          ...
          {/* Niente `minDate`: il manager può prenotare anche per date passate
