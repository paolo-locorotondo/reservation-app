...
// Dialog di caricamento massivo prenotazioni (manager). Wizard a 4 step:
//  1. Utenti
...
//
// Open/close gestito dal parent (ManagerReservationsList) via prop `open`.
...
export function ManagerBulkBookingsDialog({
  open,
  users,
  onClose,
  onSuccess,
  ...
  >(new Map());
  // Filtri di lookup correnti (condivisi, default ultimo usato dalil manager).
  // Per semplicità tutti gli utenti usano lo stesso sito+piano+tipo per
  // restringere il combo — il manager tipicamente assegna posti della stessa
  ...
  useEffect(() => {
    ...
    // qui non ci interessa la disponibilità). Usiamo oggi: il manager assegna
  ...
  async function handleSubmit() {
    ...
    setError(null);
    try {
      const res = await api.managerBulkCreateReservations({
...
function ReportView({ report, users }: ReportProps) {
  ...
  // disponibile (es. contesto non-secure) — il manager può sempre screenshottare.
